...
import { fetch_product_admin_service, get_order_admin_service, fetching_user_service, change_userStatus_Service, change_blocked_Status_Service, del_Users_Service, del_product } from '../services/Admin_services';
...
function AdminDashboard() {
    ...
    const fetch_products = async () => {
        ...
        try {
            ...
            const { product, totalProducts, totalPages, count_prd, outStock, topRated } = await fetch_products_service(param)
            set_Products(product)
            setTotalPages(totalPages)
            set_prd_loader(false)
            setTotalPrd(count_prd)
            setOut_of_stock(outStock)
            setRated(topRated)
    ...
    const fetch_order = async () => {
        ...
        try {
            ...
            const { success, order, totalRevenue } = await get_order_admin_service(params)
            if (success) {
                setord(order)
                setEarnAmount(totalRevenue)
            }
        } catch (err) {
    ...
    const handle_del_product = async (id) => {
        setLoader(true)
        try {
            const { message, success } = await del_product(id)
            if (success) {
    ...
    return (
        <PageWrapper>
            ...
            <div className='w-full h-max xb_sh bg-white px-5 flex justify-between items-center fixed py-3 top-0 right-0 z-20  '>
                {
                    user ? (
                        <Link to={"/cart"} className='w-max h-[35px] px-3 xb_sh rounded-full bg-black flex text-white justify-center gap-2 font-bold text-sm items-center cursor-pointer hover:bg-black/90 transition-all duration-150 ease-out ' > <IoBag /> Cart Items </Link>
                    ) : (
                        <h1 className='font-bold uppercase ' >As_Shop 🎯</h1>
                    )
                }
                ...
                <div className='flex items-center gap-3 '>
                    {
                        user ? (
                            <>
                                <div className='w-max px-3 bg-red-500 h-[35px] rounded-full xb_sh flex justify-center gap-2 items-center text-white cursor-pointer hover:bg-red-500/90 transition-all duration-150 ease-out font-bold text-sm '> <IoHeart /> Wishlist </div>
            ...
            <Outlet context={{
                ord, handle_cancel_order, allusers, orderstatus, setorderstatus, search_order, set_searchOrder, role, setRole, userstatus, setuserstatus, search_user, set_search_user, loader, totalprd, out_of_Stock,
                handle_userstatus, handle_block_user, handle_del_user_by_Admin, handleBrandChange, searchTerm, setSearchTerm, set_current_page, focus, setfocus, sort, setSort, category, setCategories, allBrands, rated
                , slct_brand, max_prc, setMax_prc, nav_list, prd_loader, products, totalPages, current_page, page_loader, categories, cart, setCategory, set_slct_brand, handle_del_product, EarnAmount, loader_2
            }} />
        </PageWrapper>
    )
}
