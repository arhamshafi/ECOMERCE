...
function Detail() {
    ...
    const slct_prd = sessionStorage.getItem("prd_detail") ? JSON.parse(sessionStorage.getItem("prd_detail")) : null
    ...
    return (
        <PageWrapper>
            ...
            <div className='w-full h-max xb_sh bg-white px-5 flex justify-between items-center fixed py-3 top-0 right-0 z-20  '>
                <button className='py-1 pl-2 pr-4 hover:scale-105 transition-all duration-200 ease-in-out cursor-pointer tw_sh tracking-[2px] active:scale-100 xb_sh text-sm text-white flex items-center gap-2 bg-black rounded-lg' onClick={() => { navigate("/product"), setTimeout(() => sessionStorage.removeItem("prd_detail"), 1000) }} > <IoChevronBack /> Back</button>
                ...
                <div className='w-full min-h-screen px-5 py-[70px] '>
                    <div className='w-[95%] h-[500px] pt-4 px-10 bg-white xb_sh rounded-xl mx-auto mt-6 gx_sh flex gap-5 ' >
                        ...
                        <div className='w-[40%] h-full flex  justify-center items-center flex-col '>
                            <div className='w-full h-[60%] rounded-xl overflow-hidden bg-gray-100 gx_sh relative  '><img src={slct_prd?.images[img_idx]?.url} className='w-full h-full object-contain  ' alt="" /></div>
                            <div className='w-full h-max mt-5 flex gap-2 flex-wrap'>

                                {slct_prd.images.map((img, i) => {
                                    return (
                        ...
                        <div className='w-[57%] h-full px-5 '>
                            <h1 className='font-bold text-black tb_sh text-3xl mt-10 '>{slct_prd?.name}</h1>
                            {/* <div className='w-full mt-5 flex items-center capitalize gap-1 '> <FaStar className='text-yellow-400' /><FaStar className='text-yellow-400' /><FaStar className='text-yellow-400' /><FaStar className='text-yellow-400' /><FaStar className='text-black/30' />  </div> */}
                            ...
                            <p className='text-2xl tg_sh font-bold mt-5 text-green-500 '>$ {slct_prd?.discountedPrice} <span className='text-black/30 text-sm line-through ml-1 ' style={{ textShadow: "none" }} >$ {slct_prd?.price} </span> <span className='w-max px-2 py-[3px] rounded-sm text-[13px] ml-5 font-normal bg-red-500/20 rx_sh text-red-600 ' style={{ textShadow: "none" }} >OFF {slct_prd?.discount}% </span>  </p>
                            <p className='text-black/80 mt-5 text-lg font-normal  '>{slct_prd?.description}</p>
                            ...
                            <div className='w-full flex items-center gap-3 h-max mt-7 '>
                                <button className='w-[60%] cursor-pointer hover:scale-102 transition-all duration-200 ease-in-out py-2 rounded-md back text-white bg-orange-500 xo_sh text-xl font-bold tracking-[1px] active:scale-100 '> 🛒 Add To Cart </button>
                                        ...
                                        <div key={idx} className='w-full flex bg-white rounded-2xl py-4 pl-5 xb_sh mt-4'>
                                            ...
                                            <div className='w-[90%] ml-[2%] h-max '>
                                                <div className='flex justify-between items-center  '>
                                                    ...
                                                    <h1 className='text-black font-bold text-left text-md '> {ele?.email} User Name </h1>
                                                    <div className={`flex items-center gap-1 mt-0.5 text-gray-400 `} >
                                                        {[1, 2, 3, 4, 5].map((star) => (
                                                            <FaStar key={star} className={`text-xl ${star <= ele?.rating ? "text-yellow-400" : "text-black/40"} `} />
                                                        ))}</div>
                                                </div>
                                                <p className='text-gray-500 text-left mt-[-1px] text-sm '>{ele?.email} emailhere@gmail.com </p>
