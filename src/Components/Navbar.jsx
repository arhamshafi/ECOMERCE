...
function Navbar() {
  ...
  // bar bar render hoti ha wish list

  const navigate = useNavigate()
  const [nav_list, set_nav_list] = useState(false)
  // const [wish_list , Set_Wish_list] = useState(false)
        ...
        <div className='flex items-center gap-2 '>
          {
            user ? (
              <>
                ...
                <div className='w-[35px] h-[35px] rounded-full bg-white ob_sh flex xb_sh justify-center items-center text-red-500  cursor-pointer hover:bg-white/70 transition-all duration-150 ease-out '> <IoHeart /> </div>
                <div onClick={() => set_nav_list(prev => !prev)} className={`min-w-[35px] cursor-pointer transition-all ease-in-out duration-200 h-[35px] rounded-2xl bg-white flex items-center ${nav_list ? "justify-center" : "gap-2 pl-3 pr-1"}  `}  > {nav_list ? <FaXmark /> : <> <p className='text-[15px] capitalize font-bold' >{user?.name}</p> <div className='w-[28px] h-[28px] xb_sh rounded-full overflow-hidden '> <img src={user?.avatar || "./avatar.jpeg"} alt="" className='w-full h-full' />  </div> </>} </div>
